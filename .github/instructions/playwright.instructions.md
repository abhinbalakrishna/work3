...
- **Clarity**: Use descriptive test and step titles that clearly state the intent. Add comments only to explain intent, complex logic, or non-obvious interactions; never restate the action or assertion.
...
test.describe('Movie Search Feature', () => {
  test.beforeEach(async ({ page }) => {
    // The shared setup keeps each scenario isolated at the entry point.
  ...
  test('Search for a movie by title', async ({ page }) => {
    ...
    await test.step('Verify search results', async () => {
      // The snapshot protects the accessible structure, not just visible text.
